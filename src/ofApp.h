...
public:
    ...
    void setup()
    {
        ...
        ofDirectory dir;
        dir.listDir(ofToDataPath("movies/openFrameworks"));
        dir.sort();
        //int j = 0;
        for(int j = 0; j <3; j++){
            stringstream src;
        ...
        for(int i = 0; i < maxVids*2; i++){
            ...
            if(i < maxVids){
                ...
                isf2.setImage("image"+ofToString(i), normalized[i]->getTexture());
            }
        }
        ...
        if(recording){
            ...
            if(searchQueue.size() > 0 && youTubeMap[searchQueue.front()].bReady && !youTubeMap[searchQueue.front()].bPlaying){
                ...
                if(currentTime > currentSearch.captureDuration && fade <= 0){
                    ...
                    for(int i = 0; i < foomax*2; i++){
                        players[i]->load(ofToDataPath("movies/"+youTubeMap[searchQueue.front()].files[i]));
                        players[i]->setLoopState(OF_LOOP_NORMAL);
                        players[i]->play();
                ...
                if(youTubeMap.size() > 0 && loopset.size() > 0 && youTubeMap[loopset[loopIndex]].bReady && !youTubeMap[loopset[loopIndex]].bPlaying){
                    ...
                    if(currentTime > currentSearch.captureDuration && fade <= 0){
                        ...
                        for(int i = 0; i < foomax*2; i++){
                            players[i]->load(ofToDataPath("movies/"+currentSearch.files[i]));
                            players[i]->setLoopState(OF_LOOP_NORMAL);
                            players[i]->play();
                            players[i]->setVolume(0.0);
                            players[i]->setSpeed(1.0);
    ...
    void draw()
    {
        ...
        if(debug){
            ofNoFill();
            ofDrawRectangle(0, 0, ssWidth, ssHeight);
            ofFill();
        }
        ...
        ofDrawLine(column.x, column.y+column.height+85/4, column.x+(ssWidth-column.x*2.0), column.y+column.height+85/4);
        ...
        ofDrawRectangle(column.x, column.y+column.height+25, ssWidth-(column.x*2.0), 25);
        
        ofSetColor(255, 255, 255, 100);
        ofDrawRectangle(column.x, column.y+column.height+25, ofMap(currentTime, 0, currentSearch.captureDuration, 0, ssWidth-(column.x*2.0), true), 25);
    ...
    void generateShaders(int numVids){
        for(int j = 0; j < 2; j++){
            stringstream src;
            src<<"/*{\n \"DESCRIPTION\": \"RGB Average\"\n,\"CREDIT\": \"by dantheman\"\n,\"CATEGORIES\": [\"stylized Effect\"]\n,\"INPUTS\": [";
